 ...
 *  deviceToken: '', //deviceToken 和 device_token_list 只需要也只能存在一个
 *  payload: {  // 必选，描述投递消息的JSON结构体，描述PUSH消息的：类型、内容、显示、点击动作、报表统计和扩展信息具体参考下面的详细说明。
 *   hps: { //必选，华为Push消息总结构体
 *     msg: { //必选
 *       type: 1, //取值含义和说明: 1 透传异步消息, 3 系统通知栏异步消息; 此处必须是3，应用层如果传别的值，也会被强制覆盖；
 ...
 */
make(exports, 'pushTransMsg', function (data, callback) {
    ...
    if (!checkRtn) {
        ...
        extend(options, data);
        options.payload.hps.msg.type = 1;
        
        var url = this.pushAPI;
        this.request(url, util.postJSON(options), wrapper(callback));
    }
  });